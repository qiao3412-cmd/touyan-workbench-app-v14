...
def main():
    ...
    if n != 1:
        ...
        sys.exit(3)
    sub_new = '<span class="hot-head-sub"><span>最后获取资讯时间</span><span>%s</span></span>' % ts
    html2, sn = re.subn(r'<span class="hot-head-sub">[\s\S]*?</span>', sub_new, html2, count=1)
    if sn != 1:
        print("WARN: 副标题未替换 (n=%d)" % sn)
